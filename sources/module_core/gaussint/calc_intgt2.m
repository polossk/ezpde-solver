function [ret] = calc_intgt2(vertices, basis_indices, coef_fun, meshes_ids, basis_config, diff_orders)
	...
	phi_a = FE_local_basis_2D(x_, y_, vertices, basis_indices(1), basis_config.type(ida), [r, s]);
	phi_b = FE_local_basis_2D(x_, y_, vertices, basis_indices(2), basis_config.type(idb), [p, q]);
	c = w_ .* coef_fun(x_, y_);
